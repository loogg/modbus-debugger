...
> 2026-09 审计更正：下方旧勾选曾存在覆盖不足（仅弹窗、IPC 代替 UI、宽松结果断言）。本次发现实际缺陷，历史“全通过”不能解释为每个按钮和失败路径均已验收。当前范围、修复和验证边界以 [功能审计记录](full-audit.md) 为准。
...
- [x] 下载后确认“安装更新并重启”，保存工作区并结束记录/通信；目录/ZIP、Portable、Setup 三种运行形式真实切换版本并自动重启。
- [x] 中文/空格路径、自选数据目录、Portable 外层改名、工作区、时区偏好、历史库和未知用户文件保留。
- [x] 文件清单与 SHA-256 校验；ZIP 越界、用户文件冲突、不可启动的新 EXE 拒绝或回滚。
- [x] 安装器部分写入并失败后，恢复程序文件与安装注册表版本，重新启动旧版本。
- [x] 相关静态检查、47 项 Unit/Integration/组件检查和 5 项关于页 E2E 完成，范围与测试边界见 `docs/full-audit.md` 的 0.10.1 记录。
- [x] 最终四种产物启动、清单一致性、Portable 持久化和 Setup 安装/重装/卸载完成；安装测试目录短暂占用的失败记录及定向补验见审计记录。

### 模板与数据块（0.10.2 定向验收）

- [x] 模板/数据块两级导航，返回概览和跨模块进入模板时不残留旧编辑状态。
- [x] 离线可见点位内存占用，支持多寄存器、字节/位、重叠及分页；新增点位立即反映到布局。
- [x] 数据块保存不弹文件框且真实持久化；模板概览改名、删除确认及关联清理有实际断言。
- [x] 13 项相关 Unit/Integration/组件检查和两组打包版 UI 流程通过，检查标准及紧凑窗口；未运行无关业务全量验收。
