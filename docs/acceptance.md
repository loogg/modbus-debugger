...
- [ ]  RTU 覆盖：单帧多次 read、连续多帧、半包+后续完整帧、CRC 错、噪声、截断帧、超长数据、坏帧后紧跟正常帧；使用可控时钟验证帧内 `> t1.5` 被判为不完整、帧间 `≥ t3.5` 可建立新边界，并证明下一可信帧可以恢复。
...
- [ ]  RTU Timeout / 截断 / 严重帧错误后执行 bounded drain / quiet recovery；在重新达到可信静默边界前不启动或不接受下一请求响应，并有“迟到旧响应 + 后续同 Unit/同 FC 新请求”的回归用例。
...
- [ ]  Point Mapping 的 Int/UInt/Float/Bool/Enum/String、Endian、UInt8 高低字节、BitField、String encoding/length 有 Golden Test。
...
- [ ]  Production Build 成功；打包版实际启动且 `serialport` / `better-sqlite3` native module 正常。
- [ ]  Installer / distributable Smoke Test 和 Final Full Regression 通过后才允许结束。