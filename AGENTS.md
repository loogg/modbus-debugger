...
- 独立 PyModbus 从站必须同时提供 RTU / TCP CLI。RTU 用 `--serial-port` 显式指定从站端口，禁止写死 COM 号；上位机使用虚拟串口对另一端或实际串口。TCP 用 `--host` / `--port` 指定监听地址。COM1/COM2 仅是某台机器的配对示例，不是工程默认值。
- 覆盖本工具支持的 FC01/02/03/04/05/06/15/16、独立 Unit 和四类地址区、写后读回、非法地址、未知 Unit 静默、动态/静态数据及故障注入。不能用本项目 Codec 自己生成的响应作为唯一互操作证据。
- 全量测试必须先盘点页面按钮、菜单、表单和逻辑，再把每项对应到实际断言。只打开弹窗、出现文案、测试命令退出成功或旧验收勾选，均不能证明功能完成。
- UI 验收通过真实鼠标/键盘触发操作，核对确认设备值、Main 状态、文件或历史库结果；IPC 仅可用于搭建 fixture 与观察，不得替代被测按钮。文件选择器可返回预先准备的路径，但后续文件读写必须真实，并在报告说明边界。
- 断言应能抓住相反行为：例如扫描精确验证配置 Unit 集合、取消后不继续发后续请求、重新扫描替换结果；写入区分从站实例、只读/取消不发写包、成功后读回；复制模板验证新 ID；导入验证实际地址和宽度。
- 一次全量报告列出实际通过、失败、跳过与未覆盖项；RTU 环境缺失必须明确跳过，不能把 TCP/mock 结果称为 RTU 通过。不用自动重试隐藏失败。每个修复至少增加能复现该缺陷的相关回归证据，普通任务仍按影响范围测试。
- 本次审计证据维护于 `docs/full-audit.md`。`docs/acceptance.md` 的历史勾选不替代当前证据，也不意味着以后每轮都跑全量。

### 关于与版本更新

- 左侧“关于”显示 Main 的 `app.getVersion()`、当前架构与运行形式，提供 GitHub、更新日志、检查更新和下载操作。
