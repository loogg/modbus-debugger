...
import { useApp, type ModuleId } from './store/app';
...
function TopBar() {
  ...
  const online = snapshot ? Object.values(snapshot.connections).filter((c) => c.state === 'online').length : 0;
  const rec = snapshot?.recording ?? null;
    ...
    <header className="flex h-[52px] shrink-0 items-center justify-between border-b border-line bg-surface px-[18px]">
      <div className="flex items-baseline gap-4 min-w-0">
        ...
        <span className="text-xs text-ink2 truncate">
          {snapshot?.workspace.name ?? '未命名工作区'}
...
function TreeConnection(props: { connectionId: string; children?: React.ReactNode }) {
  ...
  return (
    <div className="mb-2">
      <button className="focus-ring flex w-full cursor-pointer items-center justify-between rounded px-1 py-1.5 text-left" onClick={() => setOpen(!open)}>
        ...
        <StatusDot tone={state?.state === 'online' ? 'ok' : state?.state === 'error' ? 'err' : 'idle'} label={state?.state === 'online' ? '已连接' : state?.state === 'connecting' ? '连接中' : '离线'} />
      </button>
...
function SlaveCard(props: { slaveId: string; showBlocks: boolean; selectedSlave?: boolean; selectedBlock?: string | null; onSelectSlave?: () => void; onSelectBlock?: (blockId: string) => void }) {
  const snapshot = useApp((s) => s.snapshot);
  const slave = snapshot?.workspace.slaves.find((s) => s.id === props.slaveId);
...
function RealtimeSidebar() {
  ...
  const select = useApp((s) => s.select);
  const selection = useApp((s) => s.selection);
  const selSlave = snapshot?.workspace.slaves.find((s) => s.id === selection.slaveId);
  const selTemplate = snapshot?.workspace.templates.find((t) => t.id === selSlave?.templateId);
  return (
...
function TrendSidebar() {
  ...
  const group = snapshot?.workspace.trendGroups.find((g) => g.id === selection.groupId);
  return (
    <SidebarShell title="趋势组">
      ...
      <div className="mt-4 flex flex-col gap-2">
        {snapshot?.workspace.trendGroups.map((g) => (
          <button
            key={g.id}
          ...
          >
            ...
            <div className="text-xs text-ink2 mt-0.5">{g.signals.length} 个信号{snapshot.recording?.groupId === g.id ? ' · 记录中' : snapshot.recording ? ' · 未选中' : ''}</div>
            {snapshot.recording?.groupId === g.id ? <StatusDot tone="ok" label="当前" className="mt-1" /> : null}
          </button>
        ))}
      </div>
      {group ? (
...
function HistorySidebar() {
  ...
  const sessions = (snapshot?.sessions ?? []).filter((s) => !query || s.groupName.includes(query));
  ...
  return (
    <SidebarShell title="记录会话">
      ...
      <div className="mt-6 text-xs text-ink2">数据库：history.db · {snapshot?.sessions.length ?? 0} 个会话</div>
    </SidebarShell>
  );
}
    ...
    <SidebarShell title="通信来源">
      ...
      {snapshot?.workspace.connections.map((c) => (
        <button key={c.id} className="focus-ring mb-2 w-full cursor-pointer rounded-ctl bg-surface px-3 py-2 text-left hover:bg-surface2" onClick={() => select({ connectionId: c.id })}>
          ...
          <StatusDot tone={snapshot.connections[c.id]?.state === 'online' ? 'ok' : 'idle'} label={snapshot.connections[c.id]?.state === 'online' ? `已连接 · ${c.transport === 'rtu' ? `${c.rtu?.baudRate ?? ''} ${c.rtu?.dataBits ?? ''}${c.rtu?.parity.charAt(0).toUpperCase()}${c.rtu?.stopBits ?? ''}` : c.tcp?.host}` : '离线'} className="mt-1" />
        </button>
      ))}
      <div className="mt-4 mb-2 text-xs text-ink2">从站筛选</div>
      <div className="flex flex-col gap-1.5">
        {snapshot?.workspace.slaves.map((s) => (
...
function TemplatesSidebar() {
  ...
  return (
    <SidebarShell title="设备模板">
      ...
      <div className="mt-4 flex flex-col gap-2">
        {snapshot?.workspace.templates.map((t) => {
          const bound = snapshot.workspace.slaves.filter((s) => s.templateId === t.id).length;
          return (
      ...
      <div className="flex flex-col gap-1.5 text-xs text-accent">
        ...
        <button
          ...
          onClick={async () => {
            const t = snapshot?.workspace.templates.find((x) => x.id === selection.templateId);
            if (!t) return;
...
export function App() {
  const snapshot = useApp((s) => s.snapshot);
  if (!snapshot) return <div className="flex h-full items-center justify-center text-ink2">正在加载工作区…</div>;
  return (
