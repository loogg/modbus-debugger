...
import { ChevronDown20Regular, ChevronRight20Regular, Cube16Regular } from '@fluentui/react-icons';
  ...
  return <nav aria-label="设备模板树"><ul className="mt-4 space-y-2">
    {templates.map(template=><li key={template.id}>
      ...
      {!collapsed[template.id] && <ul className="ml-4 mt-1 space-y-1 border-l border-line pl-2">
        {template.blocks.map(block=><li key={block.id}><button aria-current={templateId===template.id&&blockId===block.id?'page':undefined} className={`focus-ring flex w-full items-start gap-1.5 rounded-ctl px-2 py-2 text-left text-xs ${templateId===template.id&&blockId===block.id?'bg-accentsoft text-accent':'hover:bg-surface'}`} onClick={()=>onBlock(template.id,block.id)}><Cube16Regular aria-hidden="true" className="mt-px h-4 w-4 shrink-0"/><span className="min-w-0 break-words">{block.name}<span className="mt-1 block text-ink2">{block.start}–{block.start+block.length-1}</span></span></button></li>)}
